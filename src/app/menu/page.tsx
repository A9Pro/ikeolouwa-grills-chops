...
const MenuPage = ({ cart, setCart, onShowCart }) => {
  const [quantities, setQuantities] = useState<{ [key: number]: number }>({});
  const [selectedOptions, setSelectedOptions] = useState<{ [key: number]: { [key: string]: string } }>({});

  const addToCart = (item: CartItem) => {
    setCart(prevCart => {
      const existingItemIndex = prevCart.findIndex(cartItem => cartItem.id === item.id && cartItem.option === item.option);
  ...
  const handleOptionChange = (itemId: number, groupTitle: string, optionLabel: string) => {
  ...
  const handleAddToCart = (item: MenuItem) => {
    ...
    const itemOptions: { group: string; option: string; price: number }[] = [];

    if (item.optionGroups) {
      item.optionGroups.forEach(group => {
        const selectedLabel = selectedOptions[item.id]?.[group.title] || group.options[0].label;
    ...
    addToCart({
      ...
      quantity,
      option: JSON.stringify(itemOptions), // Store options as a JSON string
      image: item.image, // Add the image URL here
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#F8F4E3] text-[#1A1A1A] font-cormorant antialiased">
      ...
      <main className="relative z-10 pt-28 pb-16 px-6 sm:px-12 md:px-24">
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {menu.slice(0, 9).map((item) => {
            ...
            const currentPrice = item.price + (item.optionGroups?.reduce((total, group) => {
              const selectedLabel = selectedOptions[item.id]?.[group.title] || group.options[0].label;
              const selectedOption = group.options.find(opt => opt.label === selectedLabel);
            ...
            return (
              ...
              >
                <img src={item.image} alt={item.name} className="w-full h-48 object-cover rounded-lg mb-4 shadow-inner-gold" />
                <h2 className="text-3xl font-['Playfair_Display'] font-semibold mb-2 text-[#4A4A4A]">{item.name}</h2>
                <p className="text-2xl font-bold text-[#D4A017] mb-4">₦{currentPrice}</p>
                
                {/* Options Chooser */}
                {item.optionGroups && item.optionGroups.map((group) => (
                  <div key={group.title} className="w-full mb-4">
                    <p className="font-semibold text-[#666] mb-1">{group.title}:</p>
                    <select
                      ...
                      onChange={(e) => handleOptionChange(item.id, group.title, e.target.value)}
                      value={selectedOptions[item.id]?.[group.title] || group.options[0].label}
                    >
                ...
                <div className="flex items-center space-x-4 mb-4">
                  ...
                  <span className="text-xl font-semibold w-8 text-center">{quantities[item.id] || 1}</span>
                  <button
                    onClick={() => handleQuantityChange(item.id, 1)}
                    className="bg-[#D4A017] text-white rounded-full w-8 h-8 flex items-center justify-center font-bold text-xl hover:bg-[#B88C14] transition-colors"
                  >
        ...
        <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {menu.slice(13).map((item) => (
                <li
                  ...
                  className="p-8 rounded-2xl menu-item-card flex flex-col items-center text-center"
                >
                  <img src={item.image} alt={item.name} className="w-full h-48 object-cover rounded-lg shadow-inner-gold" />
                </li>
            ))}
        </ul>
        {/* --- END Small Chops Section --- */}
      </main>
      ...
      {cart.length > 0 && (
        ...
        >
          <span className="font-['Cormorant_Garamond'] font-bold">🛒 View Cart ({cart.length})</span>
        </button>
      )}
      ...
      <style jsx>{`
        ...
        .menu-item-card {
          ...
          box-shadow: 0 4px 15px rgba(0, 0, 0, 0.1), inset 0 0 10px rgba(255, 215, 0, 0.2);
          position: relative;
          overflow: hidden;
        }
...
const CartPage = ({ cart, onGoBack, onUpdateQuantity, onRemoveItem, onProceedToCheckout }) => {
  return (
    <div className="min-h-screen bg-[#F8F4E3] text-[#1A1A1A] font-cormorant antialiased flex flex-col items-center py-20">
      ...
      <div className="relative z-10 p-8 rounded-lg shadow-xl w-11/12 md:w-2/3 lg:w-1/2 border border-[#D4A017] bg-[#F8F4E3]">
        <h2 className="text-5xl font-['Playfair_Display'] font-bold mb-6 text-center text-[#D4A017]">Your Cart</h2>
        {cart.length === 0 ? (
          <p className="text-center text-xl">Your cart is empty.</p>
        ) : (
          <ul className="space-y-6">
            {cart.map((item, index) => {
              const parsedOptions = JSON.parse(item.option);
              return (
                <li key={index} className="flex flex-col sm:flex-row justify-between items-center border-b pb-4">
                  <div className="flex items-center w-full sm:w-auto mb-4 sm:mb-0">
                    <img src={item.image} alt={item.name} className="w-20 h-20 object-cover rounded-lg mr-4 shadow-inner-gold" />
                    <div className="text-left flex-1">
                      <p className="font-['Playfair_Display'] text-xl font-semibold">{item.name}</p>
                      {parsedOptions.length > 0 && (
                        <div className="mt-1 text-sm text-gray-600">
                          {parsedOptions.map((opt, i) => (
                            <span key={i} className="block">{opt.group}: {opt.option} (+₦{opt.price})</span>
                          ))}
                        </div>
                      )}
                    </div>
        ...
        <div className="mt-6 text-right">
          <span className="text-2xl font-bold">Total: ₦{cart.reduce((total, item) => total + item.price * item.quantity, 0)}</span>
        </div>
...
const CheckoutForm = ({ cart, onConfirmOrder, onGoBack }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulate sending order details to an email/phone.
    ...
    const orderMessage = `
      ...
      ---
      Order Summary:
      ${orderDetails.orderItems.map(item => `- ${item.name} x${item.quantity} (₦${item.price * item.quantity})`).join('\n')}
      ---
      Total: ₦${orderDetails.totalPrice}
    `;

    try {
      // Simulate API calls for sending email and SMS.
      ...
      // In a real app, this is where you'd have your fetch call
      // For example:
      ...
      // Assuming success after a small delay
  ...
  return (
    <div className="min-h-screen bg-[#F8F4E3] text-[#1A1A1A] font-cormorant antialiased flex flex-col items-center py-20">
      ...
      <div className="relative z-10 p-8 rounded-lg shadow-xl w-11/12 md:w-2/3 lg:w-1/2 border border-[#D4A017] bg-[#F8F4E3]">
        <h2 className="text-5xl font-['Playfair_Display'] font-bold mb-6 text-center text-[#D4A017]">Checkout</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="name" className="block text-lg font-semibold mb-1 text-[#4A4A4A]">Name</label>
            <input
              type="text"
              id="name"
              value={name}
          ...
          <div>
            <label htmlFor="phone" className="block text-lg font-semibold mb-1 text-[#4A4A4A]">Phone Number</label>
            <input
              type="tel"
              id="phone"
              value={phone}
          ...
          <div>
            <label htmlFor="address" className="block text-lg font-semibold mb-1 text-[#4A4A4A]">Address</label>
            <textarea
              id="address"
              value={address}
      ...
      <style jsx>{`
        .btn-gold-neon-capsule-secondary {
          ...
          border: 2px solid #D4A017;
          box-shadow: 0 0 8px rgba(212, 160, 23, 0.5), inset 0 0 4px rgba(212, 160, 23, 0.5);
        }
...
const CheckoutSuccessModal = ({ orderInfo, onGoBack }) => {
  return (
    <div className="fixed inset-0 z-50 bg-[#1A1A1A]/80 flex items-center justify-center p-4">
      <div className="bg-[#F8F4E3] rounded-lg p-8 shadow-2xl border-2 border-[#D4A017] text-center max-w-lg w-full max-h-[90vh] overflow-y-auto">
        <h3 className="text-3xl font-['Playfair_Display'] font-bold text-[#D4A017] mb-4">Order Placed!</h3>
          ...
          <>
            <div className="text-left space-y-4 mb-6">
              <div className="bg-[#D4A017]/10 p-4 rounded-lg border border-[#D4A017]/30">
                <h4 className="text-xl font-bold text-[#D4A017] mb-2">Order Number</h4>
                <p className="text-2xl font-mono font-bold text-[#1A1A1A]">{orderInfo.orderNumber}</p>
              </div>
              <h4 className="text-xl font-bold text-[#D4A017]">Customer Information</h4>
              <p><strong>Name:</strong> {orderInfo.customerName}</p>
              <p><strong>Contact:</strong> {orderInfo.customerPhone}</p>
              <p><strong>Delivery Address:</strong> {orderInfo.customerAddress}</p>
            </div>
              ...
              {orderInfo.orderItems.map((item, index) => {
                ...
                return (
                  <div key={index} className="border-b border-gray-300 pb-2">
                    <p className="font-semibold">{item.name} x{item.quantity} - ₦{item.price * item.quantity}</p>
                    {parsedOptions.length > 0 && parsedOptions.map((opt, i) => (
                      <p key={i} className="ml-4 text-sm text-gray-600">{opt.group}: {opt.option} (+₦{opt.price})</p>
                    ))}
                  </div>
                );
              })}
              <p className="text-lg font-bold mt-4">Total: ₦{orderInfo.totalPrice}</p>
            </div>
          </>
        )}
        <button
...
export default function App() {
  ...
  const handleGoBackFromModal = () => {
    ...
    setOrderInfo(null);
    setCurrentPage('menu'); // Go back to the menu page
  };
  ...
  return (
    <>
      {currentPage === 'menu' && (
        <MenuPage cart={cart} setCart={setCart} onShowCart={() => setCurrentPage('cart')} />
      )}
      {currentPage === 'cart' && (
        <CartPage 
          cart={cart} 
      ...
      {showSuccessModal && <CheckoutSuccessModal orderInfo={orderInfo} onGoBack={handleGoBackFromModal} />}
    </>
  );
}